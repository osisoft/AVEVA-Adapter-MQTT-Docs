...
| **TimeField**    | Optional | `string`  | The JsonPath expression to take value to use as a timestamp from a property. A valid JsonPath expression starts with `$`. <br>**Note:** The adapter generates a timestamp when `null` is specified.<br><br>Allowed value: cannot be `null`, empty, or whitespace.        |
