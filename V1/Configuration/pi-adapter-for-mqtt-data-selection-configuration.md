...
| **TimeField**    | Optional | `string`  | The JsonPath expression to take value to use as a timestamp from a property. A valid JsonPath expression starts with `$`. <br>**Note:** The adapter generates a timestamp when `null` is specified.<sup>1</sup><br><br>Allowed value: cannot be `null`, empty, or whitespace.        |
...
| **TimeFormat**   | Optional | `string`  | The time format of the timestamp value specified in the TimeField property.<br><br>Allowed value: any string that can be parsed using `TryParse()`<sup>1</sup><br><br>**Note:** If the string cannot be parsed, specify a custom datetime string or one of the following keywords: `Adapter`, `UnixTimeSeconds`, `UnixTimeMilliseconds`<br>Default value: `null`            |
| **DataFilterId** | Optional  | `string`  | The ID of the data filter. <br><br>Allowed value: any string <br>Default value: `null`            |

<sup>1</sup> If you specify **TimeField** and **TimeFormat**, the adapter automatically sets the latter to `Adapter`, which uses an adapter-supplied timestamp for the data. The timestamp is taken after the data is published to the adapter while the adapter processes it.
